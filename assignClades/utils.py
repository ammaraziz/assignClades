...
def load_relatives():
    ...
    '''
    internal_clades = {"A1b/131K": "3C.2a1b+131K",
    ...
    with open('config/clades_relative.tsv') as f:
        from csv import DictReader
        data = DictReader(f, delimiter="\t", )
        for entry in data:
            clades_relatives[entry['clade']] = entry['relative']

    return(clades_relatives, internal_clades)
...
def get_reference(input_lineage, input_gene):
    ...
    if input_lineage.lower() in lineages and input_gene.lower() in genes:
        refname = f"config/reference_{input_lineage.lower()}_{input_gene.lower()}.gb"
        try:
            ref = SeqIO.read(refname, 'genbank')
        except Exception:
            raise ValueError(
                "Error reading reference, check file is genbank format and it exists")
    else:
        raise ValueError(
